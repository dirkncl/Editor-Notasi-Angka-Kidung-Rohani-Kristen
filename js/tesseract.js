...
});function makeAbsolutePath(caller, relPath) {var Caller = caller.split("/"); var r_to_abs = relPath.split("/");Caller.pop();for (var i = 0; i < r_to_abs.length; i++) {if (r_to_abs[i] == "."){continue};if (r_to_abs[i] == "..") {Caller.pop()}else {Caller.push(r_to_abs[i])}}return Caller.join("/");};function writeFile(file_name, text) {var anch = document.createElement('a');anch.setAttribute('href', 'data:text/plain;charset=utf-8,' + encodeURIComponent(text));anch.setAttribute('download', file_name);anch.style.display = 'none';document.body.appendChild(anch);anch.click();document.body.removeChild(anch);
